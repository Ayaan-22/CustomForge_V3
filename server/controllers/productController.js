...
export const getAllProducts = asyncHandler(async (req, res) => {
  ...
  const countFeatures = new APIFeatures(Product.countDocuments(), req.query)
    .filter();
...
export const createProduct = asyncHandler(async (req, res) => {
  const { name, category, brand, originalPrice, discountPercentage, stock, image } = req.body;

  if (!name || !category || !brand || !originalPrice || !image || stock === undefined) {
  ...
  const finalPrice = parseFloat((originalPrice - (originalPrice * discountPercentage) / 100).toFixed(2));
